...
window.registerModuleCallback(function (config) {
  ...
  const topAds = document.querySelectorAll(
    '[class="_bGlmZ_container_GMk6b sbx-desktop"]'
  );
  if (topAds.length > 0) {
    topAds.forEach((e) => {
      const sponsored_Large = e.querySelector(
        '[aria-label^="Sponsored ad from"]'
      );
  ...
  const bottomAds = document.querySelectorAll(
    '[class="a-section a-spacing-base a-spacing-top-base"]'
  );
  ...
  withinSearch.forEach((e) => {
    ...
    console.log(
      "[Amazon Search (within search)] Supplier and product description: " +
        adsDescription
    );
  ...
  const withVideo = document.querySelectorAll(
  ...
  withVideo.forEach((e) => {
    ...
    videoURLs.forEach((v) => {
      ...
      console.log(
        "[Amazon Search (between search)] Video Preview URL: " + videopreview
      );
  ...
  const verticalSideAds = document.querySelectorAll(
  ...
  window.onload = function () {
    const bottomFrame = document.querySelector(".rhf-frame");
